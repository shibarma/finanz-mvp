...
export default function SetupPage() {
  ...
  // Default категорий
  const [defaultCategoryUpdating, setDefaultCategoryUpdating] = useState<Set<string>>(new Set());
  ...
  const loadCategories = async () => {
    ...
    const { data, error } = await supabase
      .from('categories')
      .select('id, user_id, kind, name, is_default, sort_order, created_at')
  ...
  const handleUpdateCategory = async () => {
    ...
    if (!editCategoryName.trim()) {
      setEditCategoryError('Название категории обязательно.');
      return;
    }
    ...
    if (error) {
      // Проверка на unique constraint (user_id+kind+name)
      if (error.code === '23505' || error.message.includes('unique') || error.message.includes('duplicate')) {
        setEditCategoryError('Категория с таким названием уже существует для этого типа.');
      } else {
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    if (error) {
      // Проверка на внешние ключи (category используется в transactions)
      if (
        error.code === '23503' ||
        ...
        setDeleteCategoryError(error.message);
      }
      return;
    }
  ...
  const handleSetDefaultCategory = async (categoryId: string, value: boolean) => {
    if (!userId) return;
    ...
    setDefaultCategoryUpdating((prev) => new Set(prev).add(categoryId));

    try {
      if (value) {
        // Сначала сбросить is_default=false у всех категорий этого kind пользователя
        const { error: resetError } = await supabase
          .from('categories')
        ...
        if (resetError) {
          setDefaultCategoryError(resetError.message);
          setDefaultCategoryUpdating((prev) => {
            const next = new Set(prev);
        ...
        // Затем установить is_default=true у выбранной категории
        const { error: setError } = await supabase
          .from('categories')
        ...
        if (setError) {
          setDefaultCategoryError(setError.message);
          setDefaultCategoryUpdating((prev) => {
            const next = new Set(prev);
            next.delete(categoryId);
            return next;
        ...
        // Просто установить is_default=false у выбранной категории
        const { error: setError } = await supabase
          .from('categories')
          .update({ is_default: false })
          .eq('id', categoryId);
  ...
  const handleReorderCategory = async (categoryId: string, direction: 'up' | 'down') => {
    ...
    // Найти индекс текущей категории в массиве данного kind
    const categoriesOfKind = categories.filter((c) => c.kind === category.kind);
    const currentIndex = categoriesOfKind.findIndex((c) => c.id === categoryId);

    if (currentIndex === -1) return;

    // Определить соседний индекс
    let neighborIndex: number;
    if (direction === 'up') {
      if (currentIndex === 0) return; // Уже первая
      neighborIndex = currentIndex - 1;
    } else {
      if (currentIndex === categoriesOfKind.length - 1) return; // Уже последняя
      neighborIndex = currentIndex + 1;
    }

    const neighborCategory = categoriesOfKind[neighborIndex];
    if (!neighborCategory) return;

    setReorderError(null);
    setReorderingCategoryId(categoryId);

    try {
      // Получить текущую сессию для user_id
      const { data: sessionData, error: sessionError } = await supabase.auth.getSession();

      if (sessionError || !sessionData?.session?.user?.id) {
        setReorderError('Не удалось получить сессию пользователя.');
        setReorderingCategoryId(null);
        return;
      }

      const currentSortOrder = category.sort_order;
      const neighborSortOrder = neighborCategory.sort_order;

      // Swap sort_order: два UPDATE
      ...
      if (error1) {
        setReorderError(`Ошибка при обновлении порядка: ${error1.message}`);
        setReorderingCategoryId(null);
        return;
      }
      ...
      const { error: error2 } = await supabase
        ...
        .eq('id', neighborCategory.id);

      if (error2) {
        setReorderError(`Ошибка при обновлении порядка: ${error2.message}`);
        setReorderingCategoryId(null);
        return;
      }
  ...
  const handleClearPeriod = async () => {
    // Валидация
    if (!dateFrom || !dateTo) {
      setClearPeriodError('Обе даты обязательны.');
      return;
    }

    if (dateFrom > dateTo) {
      setClearPeriodError('Дата начала должна быть меньше или равна дате окончания.');
      return;
    }
    ...
    // Подтверждение
    const confirmed = window.confirm(
      `Удалить операции с ${dateFrom} по ${dateTo}? Это нельзя отменить.`,
    );
    ...
    try {
      ...
      // Безопасное формирование ISO дат
      ...
      // Удалить транзакции в диапазоне
      const { error: transactionsError } = await supabase
        .from('transactions')
        .delete()
        .eq('user_id', userId)
      ...
      // Успех
      alert('Операции за период удалены');
      // Обновить данные на странице (refetch)
      await Promise.all([loadAccounts(), loadCategories()]);
    } catch (error) {
      setClearPeriodError(
        error instanceof Error ? error.message : 'Произошла неизвестная ошибка',
                ...
                <div className="max-h-40 space-y-1 overflow-auto rounded-lg border border-neutral-200 p-3 text-sm">
                  {incomeCategories.length === 0 ? (
                    ...
                    incomeCategories.map((cat, index) => (
                      <div key={cat.id} className="rounded-md border border-neutral-200 px-2 py-2">
                        {editingCategoryId === cat.id ? (
                          <div className="space-y-2">
                            <input
                              ...
                              className="w-full rounded-lg border border-neutral-300 px-2 py-1 text-xs outline-none transition focus:border-neutral-500 focus:ring-2 focus:ring-neutral-200 disabled:bg-neutral-100"
                              autoFocus
                            />
                            {editCategoryError && (
                              <div className="rounded-lg bg-red-50 px-2 py-1 text-xs text-red-700">
                            ...
                            <div className="flex gap-2">
                              ...
                              >
                                {editCategorySubmitting ? 'Сохранение...' : 'Сохранить'}
                              </button>
                              <button
                                type="button"
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  disabled={
                                    ...
                                    index === 0 ||
                                    defaultCategoryUpdating.has(cat.id)
                                  }
                                ...
                                >
                                  {deletingCategoryId === cat.id ? 'Удаление...' : 'Удалить'}
                                </button>
                              </div>
                            </div>
                            <label className="flex items-center gap-2 text-xs">
                              <input
                                ...
                                onChange={(e) => handleSetDefaultCategory(cat.id, e.target.checked)}
                                disabled={
                                  defaultCategoryUpdating.has(cat.id) ||
                                  reorderingCategoryId === cat.id
                                }
                                className="h-3 w-3"
                              />
                              <span className="text-neutral-700">По умолчанию</span>
                            </label>
                          </div>
                        )}
                      </div>
          ...
          <div className="space-y-6">
            <div>
              <p className="mb-2 text-sm text-red-800">
                Очистить всю историю операций. Все доходы, расходы и переводы будут удалены.
                Счета и категории сохранятся.
              </p>
              {clearHistoryError && (
              ...
              <button
                ...
                className="rounded-lg border-2 border-red-500 bg-red-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-red-400 disabled:border-red-300"
              >
            ...
            <div className="border-t border-red-300 pt-6">
              <p className="mb-3 text-sm text-red-800">
                Удалит доходы/расходы/переводы в выбранном диапазоне дат. Счета и категории
                сохранятся.
              </p>
              {clearPeriodError && (
              ...
              <div className="mb-3 grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-red-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-red-500 focus:ring-2 focus:ring-red-200 disabled:bg-red-100 disabled:cursor-not-allowed"
                  />
                </div>
                <div className="space-y-1">
                  <label
